...
namespace AirStrikeKit
{
    ...
    public class FlightSystem : DamageManager
    {
        ...
        {
            AkSoundEngine.PostEvent("stopStarAlarm", gameObject);
            ...
            if (isThisPlayer == true)
             {
                ...
                if (alreadyAlertedplayer == false)
                {
                    if (HP <= 50 && HP > 0)
                    {
                        ...
                        {
                            AkSoundEngine.PostEvent("startModernAlarm", gameObject);
                            alreadyAlertedplayer = true;
                        }
                        //play sound
                        if (musicScript.currentMode == 3)
                        {
                            AkSoundEngine.PostEvent("startStarAlarm", gameObject);
                            alreadyAlertedplayer = true;
                        }
                    }
                    if (HP <= 0)
